...
const CatatanMingguan = () => {
    ...
    const fetchCatatan = async () => {
        try {
            const catatanRef = collection(db, 'CM-CNS');
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            try {
                await deleteDoc(doc(db, 'CM-CNS', id));
        ...
        <div className="container-fluid flex-col sticky h-screen mt-14 mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div className="bg-gray-100 p-3 shadow rounded-lg mb-6">
                <nav className="text-gray-600">
                    ...
                    <span className="mx-2">/</span>
                    <span>List Data Pemeliharaan Mingguan CNS</span>
                </nav>
            </div>
                    ...
                    <tbody className="text-black">
                        {filteredCatatan.slice(0, entriesPerPage).map((catatan) => (
                            <tr key={catatan.id}>
                                ...
                                <td className="py-2 px-4 border border border-gray-300 max-w-[300px]">
                                ...
                                <td className="py-2 px-4 border border-gray-300 whitespace-nowrap overflow-hidden overflow-ellipsis">{catatan.note || '-'}</td>
                                <td className="py-2 px-4 border border-gray-300 whitespace-nowrap overflow-hidden overflow-ellipsis text-center">
